...
namespace Mtelligent.Dashboard.Data
{
    ...
    {
        public const string GetCohorts = @"Select * from Cohorts (nolock) where Active=1";
        public const string GetCohort = @"Select * from Cohorts (nolock) where Id = @CohortId; Select * from CohortProperties where CohortId = @CohortId";
        public const string AddCohort =
                ...
                select * from Cohorts (nolock) where Id = scope_Identity(); ";
