...
namespace Mtelligent.Dashboard.Data
{
    ...
    {
        public const string GetExperiments = @"Select *, B.Segments, C.Name 'TargetCohortName' from Experiments A left join (select ExperimentId, count(*) 'Segments' from ExperimentSegments group by ExperimentID) B on A.Id = B.ExperimentID inner join Cohorts C on A.TargetCohortId = C.ID where A.Active=1";
        public const string GetExperiment = @"Select A.*, C.Name 'TargetCohortName' from Experiments A inner join Cohorts C on A.TargetCohortId = C.Id where A.Id = @ExperimentId; Select * from ExperimentSegments where ExperimentId = @ExperimentId; Select * from ExperimentVariables where ExperimentID = @ExperimentId; Select ExperimentSegmentId, Name, Value from ExperimentVariables A inner join ExperimentSegmentVariableValues B on A.Id = B.ExperimentVariableId Where A.ExperimentID = @ExperimentId";
