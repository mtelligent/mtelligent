...
namespace Mtelligent.Dashboard.Data
{
    ...
    {
        public const string GetSites = @"Select * from Sites where Active=1; Select * from SiteUrls where Active=1";
        public const string GetSite = @"Select * from Sites where Id = @SiteId; Select * from SiteUrls where Active=1 and SiteId = @SiteId";
        public const string AddSite =
                @"Insert into Sites (Name, Created, CreatedBy) Values (@Name, getDate(), @CreatedBy);
                select * from Sites where Id = scope_Identity()";
        ...
        public const string AddSiteUrl = @"Insert into SiteUrls (Url, SiteId, Created, CreatedBy) Values (@Url, @SiteId, getDate(), @CreatedBy);
                                           select * from SiteUrls where Id = scope_Identity()";
